...
def run_test(algorithm_name):
    """Run handover test with specified algorithm - NO GUI version"""
    info(f"\n{'='*60}\n")
    ...
    # DON'T plot graph - this avoids the threading issue
...
def plot_comparison(ssf_metrics, llf_metrics):
    ...
    ssf_handovers_str = str(ssf_handovers) if ssf_handovers else "[]"
    llf_handovers_str = str(llf_handovers) if llf_handovers else "[]"
    ...
    • Positions: {ssf_handovers_str}                         • Positions: {llf_handovers_str}
